...
def inpaint(
    ...
    disp_h, disp_w = image_and_mask["display_wh"]
    ...
    if not has_stroke:
        return base_512, "⚠️ Draw a mask first"

    cfg = CONFIGS[config_name]
    device: torch.device = pipes["device"]

    if cfg["lora"] and cfg["cn"]:
        pipe = pipes["lora_pipe_cn"]
    elif cfg["lora"]:
        pipe = pipes["lora_pipe_base"]
    elif cfg["cn"]:
        pipe = pipes["pipe_cn"]
    else:
        pipe = pipes["pipe_base"]
    ...
    lp = masked_lpips(base_512, result, mask_512, pipes["lpips_model"])
